...
class QueryHandler{
    ...
    //===============================================================================
    //Any Statment Handler
    //===============================================================================
    static public function AnyStateMentHandler($statment , $valuesArrayForAlias = array()){
        try{
            if(self::$connectionObject == null){
                throw new Exception("You must open the connection with database first !");
            }
            $Query = self::$connectionObject->prepare($statment);
            $resultOfQuery = $Query->execute($valuesArrayForAlias);
...
// $user = array("Name" => "Muhammed" , "PhoneNumber" => "05375423731" , "Username" => "khaled1231" , "Email" => "aldroubim77@gmail.com" , "Password" => '$2y$10$MMerzHeZ/NxlTaDuToEVFeYs8XLsa.TQJ/9J7caQYW0eXgKUCljbW');
// QueryHandler::openConnection("localhost" , "any_dbname" , "root" , "");
// QueryHandler::closeConnection();

// QueryHandler::insertIntoableByValues("users" , $user);
//$userId = QueryHandler::getItemInfoBySelectStatement("select Id from users order by Id Desc limit 1" , array())["Id"];
//echo QueryHandler::updateByqueryStatment("update users set Username = ? where Id = ? ;" , array("Muhammed123@" , $userId) );

// echo QueryHandler::isItemFoundQueryByValuesAndOperators("users" , $user , array("="));